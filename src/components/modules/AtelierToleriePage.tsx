...
import {
  PieceConfig, Pli, Trou, Encoche, Chanfrein, Marquage, Annotation, Point2D,
...
const STEPS = [
  ...
  { id: 2, label: 'Dessin', icon: PenLine },
    ...
    <div className="h-full flex flex-col gap-2">
      ...
      <div className="flex-1 flex gap-2.5 min-h-0">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          {/* Show FreeDrawCanvas when in Dessin step */}
          {step === 2 ? (
            <FreeDrawCanvas
              outline={piece.outline || []}
              trous={piece.trous}
              onOutlineChange={(newOutline) => {
                update({ outline: newOutline, formeBase: 'custom' });
              }}
              onTrousChange={(newTrous) => update({ trous: newTrous })}
              onBoundsChange={(w, h) => update({ largeur: Math.round(w), hauteur: Math.round(h) })}
              darkCanvas={darkCanvas}
              showGrid={showGrid}
              epaisseur={piece.epaisseur}
              className="flex-1 rounded-xl overflow-hidden border border-[var(--border-secondary)]"
            />
          ) : (
          <>
          {/* Toolbar */}
...
// ═══ STEP 2 — DESSIN LIBRE + FORMES RAPIDES ═══

function Step2({ piece, update }: { piece: PieceConfig; update: (p: Partial<PieceConfig>) => void }) {
  const hasOutline = piece.outline && piece.outline.length >= 3;

  // Quick shape generators → outline
  const makeRect = (w: number, h: number) => {
    update({ outline: [{ x: 0, y: 0 }, { x: w, y: 0 }, { x: w, y: h }, { x: 0, y: h }], largeur: w, hauteur: h, formeBase: 'custom' });
  };
  const makeL = (w: number, h: number, b: number) => {
    update({ outline: [{ x: 0, y: 0 }, { x: w, y: 0 }, { x: w, y: b }, { x: b, y: b }, { x: b, y: h }, { x: 0, y: h }], largeur: w, hauteur: h, formeBase: 'custom' });
  };
  const makeU = (w: number, h: number, d: number) => {
    update({ outline: [{ x: 0, y: 0 }, { x: d, y: 0 }, { x: d, y: h - d }, { x: w - d, y: h - d }, { x: w - d, y: 0 }, { x: w, y: 0 }, { x: w, y: h }, { x: 0, y: h }], largeur: w, hauteur: h, formeBase: 'custom' });
  };
  const makeT = (w: number, h: number) => {
    const tw = w * 0.3, mid = w / 2;
    update({ outline: [
      { x: 0, y: 0 }, { x: w, y: 0 }, { x: w, y: h * 0.35 },
      { x: mid + tw / 2, y: h * 0.35 }, { x: mid + tw / 2, y: h },
      { x: mid - tw / 2, y: h }, { x: mid - tw / 2, y: h * 0.35 }, { x: 0, y: h * 0.35 },
    ], largeur: w, hauteur: h, formeBase: 'custom' });
  };

  // Compute outline info
  let perim = 0, area = 0;
  if (hasOutline) {
    const pts = piece.outline!;
    for (let i = 0; i < pts.length; i++) {
      const j = (i + 1) % pts.length;
      perim += Math.sqrt((pts[j].x - pts[i].x) ** 2 + (pts[j].y - pts[i].y) ** 2);
  ...
  return (<>
    {/* Quick shapes */}
    <Card><CardBody className="p-2">
      <p className="text-[8px] font-bold uppercase mb-1.5">⚡ Formes rapides</p>
      <div className="grid grid-cols-4 gap-1">
        {[
          { icon: '▬', label: 'Rectangle', fn: () => makeRect(piece.largeur || 200, piece.hauteur || 100) },
          { icon: '⌐', label: 'Équerre L', fn: () => makeL(piece.largeur || 200, piece.hauteur || 100, 60) },
          { icon: '⊔', label: 'Profil U', fn: () => makeU(piece.largeur || 200, piece.hauteur || 120, 40) },
          { icon: '⊤', label: 'Profil T', fn: () => makeT(piece.largeur || 200, piece.hauteur || 120) },
        ].map(s => (
          <button key={s.label} onClick={s.fn} className="p-1.5 rounded border border-[var(--border-secondary)] hover:bg-[var(--accent-bg)] hover:border-[#B91C1C]/20 text-center transition-all">
            <span className="text-[13px]">{s.icon}</span>
            <p className="text-[6px] font-bold mt-0.5">{s.label}</p>
          </button>
        ))}
      </div>
      <p className="text-[7px] text-[var(--text-muted)] mt-1.5 text-center">
        ou dessinez librement sur le canvas →
      </p>
    </CardBody></Card>

    {/* Dimensions override */}
    <Card><CardBody className="p-2 space-y-1">
      <p className="text-[8px] font-bold uppercase">Dimensions de base (mm)</p>
      <div className="grid grid-cols-2 gap-1">
        <div><label className="text-[6px] text-[var(--text-muted)]">Largeur</label>
          <Input type="number" min={10} value={piece.largeur} onChange={e => {
            const w = Math.max(10, parseFloat(e.target.value) || 0);
            // Si on a un outline rectangle, recalculer
            if (piece.outline && piece.outline.length === 4) {
              const scaleX = w / piece.largeur;
              update({ largeur: w, outline: piece.outline.map(p => ({ x: p.x * scaleX, y: p.y })) });
            } else {
              update({ largeur: w });
            }
          }} className="text-[9px] font-mono" /></div>
        <div><label className="text-[6px] text-[var(--text-muted)]">Hauteur</label>
          <Input type="number" min={10} value={piece.hauteur} onChange={e => {
            const h = Math.max(10, parseFloat(e.target.value) || 0);
            if (piece.outline && piece.outline.length === 4) {
              const scaleY = h / piece.hauteur;
              update({ hauteur: h, outline: piece.outline.map(p => ({ x: p.x, y: p.y * scaleY })) });
            } else {
              update({ hauteur: h });
            }
          }} className="text-[9px] font-mono" /></div>
    ...
    {hasOutline && (
      <Card><CardBody className="p-2 space-y-1.5">
        <p className="text-[8px] font-bold uppercase">📐 Contour</p>
        <div className="grid grid-cols-3 gap-1 text-center">
          <div className="p-1 rounded bg-[var(--bg-tertiary)]">
            <p className="text-[14px] font-bold text-[#B91C1C]">{piece.outline!.length}</p>
            <p className="text-[6px] text-[var(--text-muted)]">sommets</p>
          </div>
          <div className="p-1 rounded bg-[var(--bg-tertiary)]">
            <p className="text-[10px] font-mono font-bold">{perim.toFixed(1)}</p>
            <p className="text-[6px] text-[var(--text-muted)]">périmètre mm</p>
          </div>
          <div className="p-1 rounded bg-[var(--bg-tertiary)]">
            <p className="text-[10px] font-mono font-bold">{(area / 100).toFixed(1)}</p>
            <p className="text-[6px] text-[var(--text-muted)]">surface cm²</p>
          </div>
        </div>

        {/* Vertices list */}
        <details className="text-[8px]">
          <summary className="cursor-pointer text-[7px] font-bold text-[var(--text-muted)] uppercase">Sommets</summary>
          <div className="mt-1 max-h-32 overflow-y-auto space-y-px">
            {piece.outline!.map((p, i) => (
              <div key={i} className="flex items-center gap-1 px-1 py-0.5 rounded bg-[var(--bg-tertiary)] font-mono">
                <span className="text-[6px] text-[var(--text-muted)] w-3">#{i + 1}</span>
                <span className="flex-1">X:{p.x.toFixed(1)}</span>
                <span className="flex-1">Y:{p.y.toFixed(1)}</span>
              </div>
            ))}
          </div>
        </details>
    ...
    {/* Drawing tips */}
    <Card><CardBody className="p-2">
      <p className="text-[8px] font-bold uppercase mb-1">🎯 Raccourcis dessin</p>
      <div className="space-y-0.5 text-[7px] text-[var(--text-muted)]">
        <p><kbd className="px-0.5 bg-[var(--bg-tertiary)] rounded text-[6px]">V</kbd> Sélection — <kbd className="px-0.5 bg-[var(--bg-tertiary)] rounded text-[6px]">D</kbd> Dessiner</p>
        <p><kbd className="px-0.5 bg-[var(--bg-tertiary)] rounded text-[6px]">C</kbd> Coter — <kbd className="px-0.5 bg-[var(--bg-tertiary)] rounded text-[6px]">H</kbd> Perçage</p>
        <p><kbd className="px-0.5 bg-[var(--bg-tertiary)] rounded text-[6px]">Shift</kbd> Contrainte H/V</p>
        <p><kbd className="px-0.5 bg-[var(--bg-tertiary)] rounded text-[6px]">Dbl-clic</kbd> arête = insérer sommet</p>
        <p><kbd className="px-0.5 bg-[var(--bg-tertiary)] rounded text-[6px]">Suppr</kbd> supprimer sommet sélectionné</p>
        <p><kbd className="px-0.5 bg-[var(--bg-tertiary)] rounded text-[6px]">+/-</kbd> taille grille</p>
      </div>
    </CardBody></Card>
  </>);
}
