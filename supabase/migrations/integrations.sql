...
-- ============================================================
-- 1. EXTENSION TABLE NOTES (colonnes pour intégrations)
-- ============================================================

-- Ajouter type de note si manquant
...
CREATE TABLE IF NOT EXISTS travaux_pieces (
  ...
  source VARCHAR(20) DEFAULT 'a_definir' CHECK (source IN ('a_definir', 'stock_depot', 'stock_vehicule', 'commande', 'fourni_client')),
  vehicule_source_id UUID REFERENCES vehicules(id),
  prix_unitaire_ht DECIMAL(10,2),
  prix_vente_ht DECIMAL(10,2),
  statut VARCHAR(20) DEFAULT 'a_commander' CHECK (statut IN ('a_commander', 'reserve', 'commande', 'recu', 'installe')),
  created_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS travaux_etapes (
  ...
  statut VARCHAR(20) DEFAULT 'a_faire' CHECK (statut IN ('a_faire', 'en_cours', 'en_pause', 'termine', 'annule')),
...
CREATE TABLE IF NOT EXISTS travaux_temps (
  ...
  type_temps VARCHAR(30) DEFAULT 'travail' CHECK (type_temps IN ('travail', 'deplacement', 'preparation', 'attente')),
  description TEXT,
  jour_id UUID REFERENCES jours(id), -- Liaison feuille d'heures
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- ============================================================
-- 4. DEMANDES DE RÉAPPROVISIONNEMENT VÉHICULE
-- ============================================================
...
CREATE TABLE IF NOT EXISTS stock_demandes_reappro (
  ...
  technicien_id UUID NOT NULL REFERENCES techniciens(id),
  statut VARCHAR(20) DEFAULT 'en_attente' CHECK (statut IN ('en_attente', 'validee', 'preparee', 'livree', 'annulee')),
...
FROM travaux t
WHERE COALESCE(t.archive, FALSE) = FALSE;

-- Vue alertes stock véhicule
CREATE OR REPLACE VIEW v_alertes_stock_vehicule AS
SELECT 
  v.id AS vehicule_id,
  v.immatriculation,
  v.marque || ' ' || v.modele AS vehicule_nom,
...
-- Fonction réserver pièces travaux depuis stock
...
BEGIN
  FOR v_piece IN 
    SELECT tp.id, tp.article_id, tp.reference, tp.quantite_prevue, tp.quantite_reservee
    FROM travaux_pieces tp
    WHERE tp.travaux_id = p_travaux_id AND tp.statut = 'a_commander'
  LOOP
    -- Chercher stock disponible au dépôt
    SELECT COALESCE(quantite_stock, 0) INTO v_stock_dispo
    FROM stock_articles WHERE id = v_piece.article_id;
    
    IF v_stock_dispo >= v_piece.quantite_prevue THEN
      -- Réserver depuis dépôt
      UPDATE travaux_pieces SET 
        quantite_reservee = v_piece.quantite_prevue,
        source = 'stock_depot',
        statut = 'reserve'
      WHERE id = v_piece.id;
      
      -- Décrementer stock
      UPDATE stock_articles SET quantite_stock = quantite_stock - v_piece.quantite_prevue
      WHERE id = v_piece.article_id;
...
-- Fonction générer demande réappro automatique
...
BEGIN
  -- Créer la demande
  INSERT INTO stock_demandes_reappro (vehicule_id, technicien_id)
  VALUES (p_vehicule_id, p_technicien_id)
  RETURNING id INTO v_demande_id;
  
  -- Ajouter les lignes pour articles en alerte
  ...
  SELECT 
    ...
    a.reference,
    a.designation,
    GREATEST(sv.quantite_max - sv.quantite, 1),
    CASE 
      WHEN sv.quantite = 0 THEN 'Rupture'
      ELSE 'Stock bas (' || sv.quantite || '/' || sv.quantite_min || ')'
    END
  FROM stock_vehicule sv
  JOIN stock_articles a ON a.id = sv.article_id
  WHERE sv.vehicule_id = p_vehicule_id
  AND sv.quantite <= sv.quantite_min;
  
  GET DIAGNOSTICS v_count = ROW_COUNT;
  
  -- Si aucune ligne, supprimer la demande
  IF v_count = 0 THEN
    DELETE FROM stock_demandes_reappro WHERE id = v_demande_id;
    RETURN NULL;
  END IF;
...
-- ============================================================
-- 7. DONNÉES INITIALES - TYPES DE DOCUMENTS
-- ============================================================
...
INSERT INTO ged_types_documents (code, libelle, categorie, duree_validite_mois, obligatoire, ordre_affichage) VALUES
  ...
  ('DEVIS', 'Devis', 'administratif', NULL, FALSE, 30),
  ('FACTURE', 'Facture', 'administratif', NULL, FALSE, 31),
...
SELECT 'Intégrations installées avec succès!' AS resultat;
